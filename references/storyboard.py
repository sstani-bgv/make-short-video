...
Идея шага: ПЕРЕД нарезкой/рендером пользователь должен увидеть и подтвердить план монтажа.
План (plan.json) составляет Codex/агент из транскрипта + стиль-системы + указаний пользователя
...
def render_storyboard(plan: dict, video: Path, out_path: Path, per_row: int) -> None:
    ...
    print("  дальше: передай storyboard пользователю на подтверждение до нарезки/рендера.")
...
def main() -> None:
    ...
    if shutil.which("ffmpeg") is None:
        sys.exit("Нет ffmpeg в PATH. Поставь: brew install ffmpeg")
