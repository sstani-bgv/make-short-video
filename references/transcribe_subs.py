...
Поток: ffmpeg извлекает моно-аудио → Groq whisper-large-v3 (verbose_json + word-level
       ...
       1 субтитр-кадр, русский) → Caption-compatible JSON для Remotion-композиции
       karaoke-субтитров `Make-Reels-Video`.
...
Ключ: GROQ_API_KEY из окружения (или из .env в cwd / в ~/.claude/.env). Тот же ключ, что и
в остальной системе.

--------------------------------------------------------------------------------------------
ФОРМАТ ВЫХОДА (для Remotion Caption props)

{
  ...
  "subs": [
    {
      "index": 1,
      "start": 0.55,          # сек — начало caption-window
      "end": 2.30,            # сек
      "duration": 1.75,       # сек — длительность caption-window
      "text": "смотри что он делает сам",
      "hl": "сам",            # ключевое слово фразы → активный Marggraff token
...
Как это ложится в Remotion: каждый объект subs[i] преобразуется в Caption с
startMs/endMs/timestampMs/text/confidence. Пословные words[] используются для
активной karaoke-подсветки; `hl` остаётся семантической подсказкой для ручной проверки.
...
ИСПОЛЬЗОВАНИЕ
  ...
  GROQ_API_KEY=gsk_... python3 transcribe_subs.py <video|audio> -o subs.json [--language ru]
...
def load_api_key() -> str:
    """GROQ_API_KEY из env → .env в cwd и родителях → ~/.claude/.env."""
    ...
    for d in candidates:
        ...
        if v:
            return v
    v = _from_dotenv(Path.home() / ".claude" / ".env")
    if v:
        return v

    raise SystemExit(
        "Нет GROQ_API_KEY. Экспортируй ключ: export GROQ_API_KEY=gsk_... "
        "(или положи в .env в проекте / в ~/.claude/.env)."
    )
...
def extract_audio(src: str, out_path: Path) -> Path:
    ...
    if shutil.which("ffmpeg") is None:
        raise SystemExit("Нет ffmpeg в PATH. Поставь: brew install ffmpeg")
...
def groq_transcribe(audio_path: Path, api_key: str, language: str) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "make-short-video/1.0 (+remotion; python-urllib)",
    }
...
def main() -> None:
    ...
    print("  дальше: преобразуй subs[] в Remotion Caption props "
          "(startMs/endMs/timestampMs/text/confidence) и прогони QA.")
